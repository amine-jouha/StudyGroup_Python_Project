...
class User(UserMixin, db.Model):
    ...
    def __repr__(self):
        return f"User('{self.id}','{self.username}', '{self.email}', '{self.password}', '{self.domain}')"
...
@login_required
def dashbord():
    return render_template('Dashio/index.html', name=current_user.username, email=current_user.email, id=current_user.id,
                                             password=current_user.password, domain=current_user.domain)
...
def signup():
    ...
    if form.validate_on_submit():
        hashed_password = generate_password_hash(form.password.data, method='sha256')
        new_user = User(username=form.username.data, email=form.email.data, password=hashed_password, domain=form.domain.data)
        db.session.add(new_user)
        db.session.commit()

        flash("user has been created","succes")
...
@login_required
def profile():
    return render_template('profile.html' , name=current_user.username, domain=current_user.domain)
